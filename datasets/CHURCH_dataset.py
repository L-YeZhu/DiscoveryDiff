...
class CHURCH_dataset(Dataset):
    def __init__(self, image_root, transform=None, mode='train', img_size=256):
        ...
        # print("check image_path:", image_root, len(self.image_paths))
...
def get_church_dataset(data_root, config):
    ...
    train_dataset = CHURCH_dataset(data_root, transform=train_transform, mode='train', 
                                 img_size=config.data.image_size)
    test_dataset = CHURCH_dataset(data_root, transform=test_transform, mode='val',
                                img_size=config.data.image_size)
